...
- **Approach is sugar over sub-opportunity assignment, not an independent tag.** Changing a vendor's Approach finds the sub-opportunity in this opportunity that already has that lever and moves the vendor into it — or creates one (default Small/Med) if none exists yet. This was a real bug caught in review: the first version of this fix let `approach` and `so` (sub-opp) drift independently, so a vendor could visually sit under one Approach group while its spend silently still counted toward a *different* sub-opp's savings math on the Derivation tab. Fixed so the roster grouping and the savings math can never disagree.
